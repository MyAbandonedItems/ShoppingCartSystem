...
public class LoginFrame extends JFrame implements ActionListener {
	...
	/**
	 * 功能：产生一个登录界�?,供用户�?�择自己的身份，
	 * 只有当用户输入正确的账户和匹配的密码之后，程序才会进入下�?界面
	 * @param:  LFpwd ,LFusr  分别从TextField 和PasswordFiled 中获取的�?
	 * @param: usrModeOnData 从单选按钮获取的对应的对象，此对象用来传递用户在单�?�按钮中选择的模式，
	 * 具体的实现类有ActualUsrMana ActualUsrClient ActualUsrCustom 三个�?
	 * @param:reset login usrMode usr pwd usrInfo pwdInfo  JRBtnGp usrModSITem 均为见名知意的JFrame中的相应组件 
	 * 其中�?要特别说明的是，JRBtnGp是一个单选按钮的组，加入这个组可以使单�?�按钮真正实现在同一时刻只能有一个被选中
	 * @param:usrModeName 是加入标签需要使用的数组�?"管理�?" "商家""客户"的顺序固定，在之后的代码中均采用此顺�?
	 * */
	private static final long serialVersionUID = 1L;
	String LFpwd;
	String LFusr;
	Object usrModeOnData;//用来传�?�使用�?�模�?
	JButton reset;
	JButton login;
	JButton regit;
	...
	private void initRadioBt(){
		...
		/* *
		 * 初始化单选按钮的相关组件
		 * *监听用户的�?�择，目前只能实现不可修改的选项�?
		 * 这意味着用户�?旦�?�错只有�?出程序再次进入才能实现改�?**/

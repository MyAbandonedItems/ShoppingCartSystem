...
class XmlFuncForCustomInfo {

	public XmlFuncForCustomInfo() {
	...
	int GoodsKindsNum;
     */  
    public static List<CustomInfo> selectXML(String filePath){  
         List<CustomInfo> clientValueList = new ArrayList<CustomInfo>();  
         try{  
             //读取传入的路径，返回一个document对象  
             Document document = XmlFunction.loadInit(filePath);  
             //获取叶节点  
             NodeList nodeList = document.getElementsByTagName("Customs");  
             //遍历叶节点  
            // System.out.println("节点数目："+nodeList.getLength());
             for(int i=0; i<nodeList.getLength(); i++){  
            	 
            	 CustomInfo clients = new CustomInfo();  

...
class MainFrame extends JFrame {
	/**
	 * 功能：从数据库中提取数据并将数据填充到JTable中
	 * step1：先将数据从数据库中提取出来放到Vector中
	 * step2：再将Vector中的数据填充到Jtable中
	 * */
	...
	private String MBname[] = {"文件","操作","帮助","关闭","最小化","最大化","添加用户","重置所填","About"};
...
private void initData(){
	/**
	 * 将数据库中的数据加入向量中,此类同样是采用文件系统，不用数据库，故之前写的部分代码被注释
	 * */
...
/**
 * 将向量中的数据加入一个数组，再加入JTable，至于为什么不直接使用向量，是因为建JTable的模型不支持Vector
 * */
		initData();
		String[] headers = {"商品名称", "商品序列号","价格", "总数", "剩余"};
		...
		for (int i =0; i < MAX_NUM_ON_TABLE&&i<size; i++) {
			...
			info[i][0]=new String(Gname.elementAt(i));//将向量中的数据填充入数组中
